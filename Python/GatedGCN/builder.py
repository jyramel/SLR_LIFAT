...
class STgraph(data.Dataset):
    def __init__(self, jsonList, seuil, sigmas = [1,1,1], kth = 9):
        
        self.nodes, self.labels, self.unique_labels = get_STgraphs(jsonList, seuil)
...
def get_class(y_test):
    Y = []
    for y in y_test :
        Y.append(y.split("_")[0])
    return np.unique(np.array(Y))